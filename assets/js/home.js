...
function initialize() {
    fetch('https://gojekone.herokuapp.com/slider', {
            method: 'GET', // or 'PUT'
            headers: {
        ...
        .then(data => {
            ...
            let sliders = "";
            $.each(data, (ele, ele1) => {
                sliders += `<div class="swiper-slide centered"><div class="container"><div class="row"><div class="col-md-8 offset-md-2"><div class="img-slide"><a href="${ele1['Redirect Link']}" class="load-spiral cursorExplore"><img class="img-fluid" src="${ele1['Slider URL']}" alt=""></a></div></div><div class="col-md-12 text-center"><div class="text-head-slide"><div class="main-title text-slide main-title-before" style="overflow: inherit;"><h2 class="text-1">${ele1['Title']}</h2><h2 class="text-2">${ele1['Title']}</h2></div></div></div></div></div></div>`
            })
    ...
    fetch('https://gojekone.herokuapp.com/new-release', {
            method: 'GET', // or 'PUT'
            headers: {
                'Content-Type': 'application/json',
            }
